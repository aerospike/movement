...
import com.aerospike.movement.util.core.iterator.IteratorWrap;
...
public final class IteratorUtils {
    ...
    public static <A, B> Iterator<Map.Entry<A, B>> zip(Iterator<? extends A> iteratorA, Iterator<? extends B> iteratorB) {
        return new Iterator<>() {
            @Override
            public boolean hasNext() {
                return iteratorA.hasNext() && iteratorB.hasNext();
    ...
    public static Iterator<?> wrap(WorkChunk chunk) {
        return new IteratorWrap<>((Iterator<?>) chunk);
    }

    public static Iterator<?> wrap(Iterator<?> idSupplier) {
        return new IteratorWrap<>(idSupplier);
    }

    public static Iterator<Object> wrap(Collection<?> collection) {
        return new IteratorWrap<>(collection.iterator());
    }

    public static <T> Map<String, List<T>> consolidateToMap(Stream<Map.Entry<String, List<T>>> stream) {
        return stream.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }

    public static <T> Map<String, List<T>> consolidateToMap(Iterator<Map.Entry<String, List<T>>> iterator) {
        return stream(iterator).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
}
