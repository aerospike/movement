...
public class JVMGlobalRuntimeMetrics {
    ...
    static final Logger logger = LoggerFactory.getLogger(JVMGlobalRuntimeMetrics.class);
    ...
    public static void reportError(final Exception error){
        logger.debug("Error encountered: {}", error.getMessage());
            ...
            airbrakeAdjustmentTask = CompletableFuture.supplyAsync(() -> {
                while (true) {
                    try {
                        ...
                        logger.error("Airbrake adjustment task interrupted");
                        break;
                    }
                }
                return null;
